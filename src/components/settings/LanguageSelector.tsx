    ...
    >
      <div className="flex items-center space-x-1">
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            className={`px-2 py-1 text-sm font-semibold bg-control border border-control-border rounded min-w-[200px] text-start flex items-center justify-between transition-all duration-150 ${
          ...
          {isOpen && !isUpdating("selected_language") && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-floating glass-blur-floating glass-raised border border-hairline rounded-xl z-50 max-h-60 overflow-hidden">
              {/* Search input */}
              <div className="p-2 border-b border-control-border">
                <input
                  ref={searchInputRef}
                  type="text"
                  value={searchQuery}
                  ...
                  className="w-full px-2 py-1 text-sm bg-control border border-hairline rounded focus:outline-none focus:ring-1 focus:ring-accent focus:border-accent"
                />
              </div>

              <div className="max-h-48 overflow-y-auto">
                {filteredLanguages.length === 0 ? (
                  <div className="px-2 py-2 text-sm text-text-muted text-center">
      ...
      {isUpdating("selected_language") && (
        <div className="absolute inset-0 bg-control rounded flex items-center justify-center">
