/* eslint-disable */
/**
 * Vendored from samasante/liquid-glass, examples/GlassSwitch.tsx (MIT).
 * https://github.com/samasante/liquid-glass/blob/main/examples/GlassSwitch.tsx
 *
 * The library ships this as an example, not as a package export: "Copy it into
 * your app and restyle it; it is not a package export." It is the reference
 * mounting for the lens — the whole control is the <Glass>, with a copy of the
 * track as its `refract` node and the pointer handlers on the thumb inside.
 * Rebuilding it from the optics constants alone does not reproduce it.
 *
 * Kept verbatim so it can be diffed against upstream. Dictus geometry and
 * colours are passed in as props from src/components/ui/, never edited here.
 */
import React, {
  useEffect,
  useLayoutEffect,
